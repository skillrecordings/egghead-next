  ...
  async ({event, step}) => {
    ...
    if (event.data.moduleSlug) {
      sanityModule = await step.run('get the module from Sanity', async () => {
        // should make _type dynamic eventually
        return await sanityQuery(
          `*[_type == "tips" && slug.current == "${event.data.moduleSlug}"][0]`,
        )
      })
    }
    ...
    // await step.run('Order Transcript [Deepgram]', async () => {
    //   return await orderDeepgramTranscript({moduleSlug:  event.data.moduleSlug, mediaUrl: event.data.originalMediaUrl, videoResourceId: event.data.fileName})
    // })
