      ...
      {planFeatures.map((feature: string) => {
        return (
          <li className="font-medium flex mb-4 w-full" key={slugify(feature)}>
            ...
            <span className="ml-4 leading-tight text-lg dark:text-black">
              {feature}
            </span>
          </li>
        )
...
function BestValueBanner() {
  return (
    <motion.div
      ...
      className="lg:relative lg:h-0 lg:w-0 lg:top-0 lg:right-6 lg:-rotate-6"
    >
      <div className="py-3 flex text-base lg:shadow items-center justify-center h-auto w-full lg:w-32 bg-yellow-300 dark:text-black lg:rounded uppercase tracking-loose font-semibold relative">
...
const MemberPricingSection: React.FunctionComponent<MemberPricingProps> = ({
  ...
  return (
    ...
    >
      <h3 className="text-2xl font-semibold text-center pb-6 border-b border-gray-300 mt-6 dark:text-black">
        Become a member
      </h3>
      {currentPlan === annualPlan && <BestValueBanner />}
      <div className="flex flex-col justify-center items-center p-4 md:p-8">
        <span className="w-full text-center font-semibold mx-auto text-lg dark:text-black">
          {currentPlan?.name}
        </span>
