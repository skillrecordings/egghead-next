...
What's nice is that we can test this code without the rest of our setup, using [ASTExplorer](https://astexplorer.net/). Using the `transform` toggle at the top select babel v7 and start writing your plugin! Let's walk through an example.
...
We can prevent that using [recast](https://github.com/benjamn/recast), so let's install that. Recast gives us functions that parse and print our code while preserving things like white space.
