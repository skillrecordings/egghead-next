...
While this is a bit extreme of an example (typically we should be more conscious about the size of the images we load), this could realistically happen in scenarios where you’re not the one managing the images getting added or simply someone forgot to check how big that image was before dropping it in.

Without much work, we’re providing an optimized image that was converted from JPG to WEBP, requiring our visitors to download less and while allowing the page to load faster.
...
The one interesting to us here is the responsive option, where not only will our image resize as the viewport changes, Next.js will provide a srcset attribute which will load different images at different sizes depending on how big the viewport is.
...
While images are just one part of how we can focus on providing a better user experience to our visitors, not paying attention could potentially lead to serving your potential customers huge images, costing them bandwidth and time, as well as costing you and your business potential sales.
