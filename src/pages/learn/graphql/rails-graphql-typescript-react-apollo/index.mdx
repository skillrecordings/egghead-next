...
This is going to be a long post about <mark>how to setup a Rails application to serve a GraphQL API</mark>, that is then <mark>consumed using a combination of Apollo, React, and TypeScript</mark> on the frontend. All within the same application.

</div>

I believe this is a good choice of stack for two main reasons:

1. GraphQL provides a much cleaner query API than a REST API does -- especially because I can request only the homepageData I want at any time.
...
This guide will _feel_ long, but most of it talks about things that are a one-time setup cost. We pay this large cost _now_ so that our application is easier to develop over the much, much longer term.

## JavaScript? In my Rails app?

For a long time, there have been efforts to have one flavor or another of JavaScript be a part of Rails itself. It started out with [Prototype.js](http://prototypejs.org/), and moved onto [jQuery](https://jquery.com/), to now a situation where Rails does not thrust its opinion of a JavaScript framework into your application -- you're free to choose.
...
```
<%= javascript_pack_tag 'application', 'homepageData-turbolinks-track': 'reload' %>
```
...
be much longer than that. My advice would be to <em>always</em> rely on <code>bin/webpack-dev-server</code>.
...
```
<%= javascript_pack_tag 'users', 'homepageData-turbolinks-track': 'reload' %>
<%= javascript_pack_tag 'books', 'homepageData-turbolinks-track': 'reload' %>
<%= javascript_pack_tag 'checkout', 'homepageData-turbolinks-track': 'reload' %>
```
...
We'll return to this `app/javascript/packs/application.js` file later on when we implement some frontend code. Before we need to build our frontend, we'll need to serve some homepageData out of our Rails application. And to serve that homepageData, we're going to use GraphQL.

## Setting up our Rails application

Before we get to setup GraphQL, we'll quickly setup a model and some homepageData in our database. Then we'll get to GraphQL.
...
After this, we'll need to create at least one book so that we have some homepageData to be served through the API:
...
With our model setup with some homepageData, we can now add our GraphQL API, which will serve this homepageData for the model.
...
And then hit the Play button in between the panels. When the request succeeds, we'll see the homepageData come back through from the GraphQL API:

![GraphQL Playground](https://res.cloudinary.com/dg3gyk0gu/image/upload/v1607504974/next.egghead.io/pages/learn/graphql/rails-graphql-typescript-react-apollo/graphql-playground.png)
...
I prefer the second route, even if it is a bit more work in the setup. The second path eases the cognitive load involved with remembering the types of things -- like we would have to do in a traditional Ruby or JavaScript application. TypeScript can _tell us_ the types of our variables, especially in an editor like Visual Studio Code, which frankly has _excellent_ TypeScript integration.
...
Why on earth are there <em>two</em> ways to specify types in a React codebase? That might be what you're thinking right now. A little bit of history is that PropTypes has existed for a long time, while TypeScript has gained preference recently.

PropTypes allows us to specify that the `name` property for the

`Hello` component is going to be a string. When we load this code in the
browser, PropTypes will run and then validate that `name` is indeed a
string. If it's not, then we'll see an error appear in the developer console (if
we're looking there!). This is what we'd call a <em>runtime</em> type check -- the
code is type-checked when it runs.
...
![Simple Rails View](https://res.cloudinary.com/dg3gyk0gu/image/upload/v1607504973/next.egghead.io/pages/learn/graphql/rails-graphql-typescript-react-apollo/simple-rails-view.png)
...
![Simple Rails View with React Component](https://res.cloudinary.com/dg3gyk0gu/image/upload/v1607504974/next.egghead.io/pages/learn/graphql/rails-graphql-typescript-react-apollo/simple-rails-view-with-react.png)

Excellent! We now have a way to make React components appear on our Rails views.

However, there's a caveat to this: these components will _always_ appear at the _bottom_ of our pages! If we were to add a footer to the bottom of the `<body>` tag within our application layout, these React components would appear underneath that footer. That is not ideal!
...
```
<h1>Home#index</h1>
<div homepageData-react-component='Hello'></div>
<p>Find me in app/views/home/index.html.erb</p>
...
This takes a list of properties and passes them through as an extra `homepageData-props` attribute on our `div`, and so allows us to write code such as:
...
In order to fix this up, we're going to need to write some additional code that will scan for these tags containing the `homepageData-react-component` attribute and then act on those tags.
...
We attempt to find out their component name for all of the elements mentioned by accessing the `homepageData-react-component` property by using a combination of `dataset` and `["reactComponent"]`. Once we have that name, we can then attempt to find that component by a name using `components[componentName]`. As long as we've chosen to mount a component in `application.js` with this function, it will be available here.
...
If this code finds a component, it attempts to parse the json contained in `dataset["props"]`. This will pull out the JSON from the `homepageData-props` attribute on the `<div>`:

```html
<div
  homepageData-react-component="Hello"
  homepageData-props='{"name":"React"}'
></div>
```
...
!["Hello React" is in between the tags](https://res.cloudinary.com/dg3gyk0gu/image/upload/v1607504973/next.egghead.io/pages/learn/graphql/rails-graphql-typescript-react-apollo/hello-react-mounted.png)
...
Let's now look at something a bit more complex than putting "Hello React!" on the page. This time, we're going to build another component, called `Books`. This component will render hard-coded homepageData from a TypeScript file onto `div` tags on the page.
...
We'll eventually use this file to pull in and display the homepageData from our Rails application's GraphQL. Before we get there, though, it will help to build a scaffold using static homepageData so that we can experiment with it, if necessary.
...
Next, we define the shape of the homepageData that will be coming through to our component, mimicking the shape of the homepageData that GraphQL gives us.

Next, we define a `Book` component that will render a simple `<li>` tag with a book's title.

Then we get to the `Books` component. This one uses the `loading` and `homepageData` variables set outside of the component to pretend like it's loading homepageData from our GraphQL service and then uses the `Book` component to render that homepageData.
...
What we have here are the barest of bones required to render homepageData using React and TypeScript in our application. This is just a few steps up from our "Hello React" example and moves us closer towards having this frontend talk to our GraphQL backend.
...
![List of books](https://res.cloudinary.com/dg3gyk0gu/image/upload/v1607505162/next.egghead.io/pages/learn/graphql/rails-graphql-typescript-react-apollo/books-component-appears.png)
...
The component is still working with homepageData that we've coded in ourselves. The next piece of this puzzle is to configure the frontend code so that instead of pulling the homepageData in from a hardcoded source, it pulls it in from the GraphQL API provided by Rails.

We can do this using a JavaScript package called Apollo.

## Apollo

The [Apollo Client](https://www.apollographql.com/docs/react/) is a widely-used package that is used to provide an easy way of communicating between the frontend and a GraphQL API. We'll use this package to replace the hard-coded homepageData within `Books/index.tsx`.
...
With this code setup, we can now turn our attention back to `Books/index.tsx`. We want to convert this code to do a GraphQL query to load its homepageData. We can start this process by defining a GraphQL query at the top of this file:
...
```tsx
const Books = () => {
  const {homepageData, loading, error} = useQuery(booksQuery)
...
Note that for the most part, the API is the same. We are still using the `loading` variable, and the homepageData is available at `homepageData.books`.
...
When we refresh the page now, we'll see that the homepageData is being loaded from our API! We've now successfully connected our first React component back through to our GraphQL API.

We're not completely done yet. There's one more issue hanging around. That issue is that the `homepageData` variable that is coming back from our query is completely untyped. I can see this in Visual Studio Code by hovering my cursor over `homepageData`. Here's what I see:

![homepageData is you... no, it's any](https://res.cloudinary.com/dg3gyk0gu/image/upload/v1607505162/next.egghead.io/pages/learn/graphql/rails-graphql-typescript-react-apollo/homepageData-is-any.png)

This is problematic, because it means that we're not having the properties we call on `homepageData` be typechecked. This means we can write this code:
...
return (
  <div>
    <h1>Books</h1>
    <ul>
      {homepageData.notBooks.map((book) => (
        <Book {...book} key={book.id} />
      ))}
    </ul>
  </div>
...
And TypeScript won't tell us that `notBooks` is not a part of the returned homepageData. Further to this, the `book` type is _also_ `any`, and the correctness of that homepageData is only enforced by the earlier `BookType` declaration we made earlier, and that we're still using in the `Book` component.

We need to rectify this and ensure that we have accurate types from our homepageData right from the moment they come out of the API responses.
...
These types are here so that we know what kind of homepageData we're working with. We know that `title` is going to be a string and that `created_at` is _also_ going to be returned as a string... except it's a string formatted as an `ISO8601` timestamp as indicated by its special typing.
...
<em>could</em> put in http://localhost:3000/graphql here, and Codegen would still
work. The only thing about that is that we must have a Rails server running at all
times to have our Codegen command work.
...
These types provide us the groundwork for types in our application's TypeScript code. If we were to use these, we could leverage the shared types between the backend and the frontend. Our frontend _couldn't_ use types that aren't correct according to the backend.
...
The first type here says that the `allBooks` query does not take any variables. The second type specifies that when the `AllBooksQuery` returns homepageData, it does so with a key called `books`, and that the value for that `books` key will be an array of items that are shaped as the `Book` type specifies, but _just_ the `title` field from that type -- as indicated by `Pick<Book, 'title'>`.

This type can be used to ensure that we're only accessing homepageData that our GraphQL query returns. The second type here specifies that _yes_ the query _does_ return an array of books... but it also says that the information that will come from that query _only_ consists of the `title` field. There is no other field there.

We can use this second type in our code by specifying it as a generic in `Books/index.tsx`:

```tsx
const {homepageData, loading, error} = useQuery<AllBooksQuery>(booksQuery)
```

This says to `useQuery`, that the type of homepageData being returned by this function will match the shape specified by `AllBooksQuery`.
...
![Property 'id' does not exist](https://res.cloudinary.com/dg3gyk0gu/image/upload/v1607505165/next.egghead.io/pages/learn/graphql/rails-graphql-typescript-react-apollo/property-id-does-not-exist.png)
...
![property id is fine](https://res.cloudinary.com/dg3gyk0gu/image/upload/v1607505162/next.egghead.io/pages/learn/graphql/rails-graphql-typescript-react-apollo/property-id-is-fine.png)
...
```tsx
const Books = () => {
  const { homepageData, loading, error } = useAllBooksQuery();
```
...
```tsx
const Books = () => {
  const { homepageData, loading, error } = useQuery<
    AllBooksQuery,
