...
export const instructorRouter = router({
  ...
  createDraftCourse: baseProcedure
    .input(
      z.object({
        title: z.string(),
    ...
    .mutation(async ({input, ctx}) => {
      const {title, description, instructorId} = input

      let sanityInstructor = await loadSanityInstructorByEggheadId(instructorId)

      const courseSlug = slugify(title.toLowerCase(), {
        remove: /[*+~.()'"!:@]/g,
      })
      ...
      let transaction = await createSanityCourse(sanityCourse)

      if (!transaction.results && transaction.response.statusCode === 403) {
        throw new TRPCError({
